...
f4 = parfeval(@syncPhotometry, 1 , mouseNum, session, sweepTime, curdir);

[reach_data, reachTimes] = reach_precision_mouse(reach_data, session, mouseNum, sweepTime, log_name);
% [reach_data, reachTimes] = reach_precision_mouse_TTL(reach_data, session, mouseNum, sweepTime, log_name, dq);
...
pulseTimes = fetchOutputs(f4);

% align reach time in behavCam time
% reach_align_result(reachTimes, behavCamTimes, camTimes, curdir, session, mouseNum);
reach_align_result_TTL(reachTimes, behavCamTimes, camTimes, pulseTimes, curdir, session, mouseNum);
